...
class TextAttentivePool(nn.Module):
    """Pools a variable-length conditioning sequence into one vector via a single learned query.

    Used for both T5 token features and MorphEm per-channel CLS tokens; `Ct5` is just
    the conditioning feature dim.
    """
    def __init__(self, Ct5: int, D: int):
        ...
        # num_heads must divide D, since CrossAttention splits its query/output dim (=D) across heads.
        # The original formula derives it from the *conditioning* dim, which only happens to divide D
        # for the released configs (2B: Ct5=D=2048 -> 16; 20B: Ct5=2048, D=4608 -> 32). Keep it whenever
        # it is valid so those checkpoints load unchanged, and fall back to deriving from D otherwise
        # (e.g. MorphEm's Ct5=384, where 2048 % (384//128) != 0).
        num_heads = Ct5 // self.head_dim
        if num_heads <= 0 or D % num_heads != 0:
            num_heads = max(1, D // self.head_dim)
            print(f'[TextAttentivePool] Ct5={Ct5} does not yield a head count dividing D={D}; using num_heads={num_heads}')
...
class Infinity(nn.Module):
    def __init__(
        self, vae_local,
        text_channels=0, text_maxlen=0,     # conditioning: feature dim and max #tokens of the cond sequence
                                            # (T5 token features, or MorphEm per-channel CLS tokens)
        ...
        # t2i==True selects the cross-attention backbone, i.e. "conditioned on an external
        # sequence" (T5 tokens or MorphEm channel-CLS tokens) rather than on a class label.
