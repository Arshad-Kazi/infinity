...
class Args(Tap):
    ...
    # dataset
    dataset_type: str = 't2i'           # 't2i': text-image jsonl streaming; 'chammi_zarr': CHAMMI single-cell zarr
    chammi_image_size: int = 0          # 0: derive from pn (dynamic_resolution_h_w[1.0][pn]['pixel'])
    chammi_datasets: List[str] = []     # subset of Allen/CP/HPA; empty = all
    chammi_channels: List[int] = []     # restrict to these channel indices; empty = all
    ...
    morphem_img_size: int = 224         # conditioning images are resized to this before MorphEm
    morphem_normalize: str = 'morphem'  # 'morphem': official preprocessing | 'none': passthrough
    morphem_saturation_noise: int = 1   # official SaturationNoiseInjector (stochastic); 0 for deterministic cond
    morphem_maxlen: int = 16            # max #channels of a conditioning image (sizes the uncond embedding)
    morphem_cond_channels: int = 0      # encode only the first N channels of the batch (0 = all).
                                        # Set to 1 for chammi_zarr, whose single channel is replicated
                                        # to RGB for the VAE -- encoding all 3 gives identical CLS tokens.
    # GPT
    ...
    Ct5: int = 2048                     # feature dimension of the conditioning encoder (T5: 2048; MorphEm: 384)
...
def init_dist_and_get_args():
    ...
    if args.cond_type == 'morphem':
        # MorphEm conditioning: the cond sequence is one CLS token per image channel, so the
        # feature dim is fixed by the checkpoint and the sequence is short. Override the
        # T5-shaped defaults so the uncond embedding and cond projections are sized correctly.
        from infinity.models.morphem_encoder import MORPHEM_EMBED_DIM
        if args.Ct5 != MORPHEM_EMBED_DIM:
            print(f'[cond] cond_type=morphem: overriding Ct5 {args.Ct5} -> {MORPHEM_EMBED_DIM}')
            args.Ct5 = MORPHEM_EMBED_DIM
        if args.tlen != args.morphem_maxlen:
            print(f'[cond] cond_type=morphem: overriding tlen {args.tlen} -> {args.morphem_maxlen}')
            args.tlen = args.morphem_maxlen
        # Keep online_t5=True so the dataset yields raw captions (which we ignore) instead of
        # trying to load precomputed T5 .npz features. The T5 model itself is never loaded;
        # see build_model_optimizer in train.py.
        args.online_t5 = True
    elif args.cond_type != 't5':
        raise ValueError(f"cond_type must be 't5' or 'morphem', got {args.cond_type!r}")
