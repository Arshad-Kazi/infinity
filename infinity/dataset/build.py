...
def build_t2i_dataset(
    ...
    if getattr(args, 'dataset_type', 't2i') == 'chammi_zarr':
        from infinity.dataset.dataset_chammi_zarr import ChammiZarrIterableDataset
        return ChammiZarrIterableDataset(
            data_path,
            pn=args.pn,
            image_size=args.chammi_image_size or None,
            batch_size=args.batch_size,
            num_replicas=tdist.get_world_size(),
            rank=tdist.get_rank(),
            dataloader_workers=args.workers,
            datasets=args.chammi_datasets or None,
            channels=args.chammi_channels or None,
            random_flip=bool(args.chammi_random_flip),
            read_threads=args.chammi_read_threads,
