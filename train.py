...
def build_model_optimizer(args, vae_ckpt):
    ...
    if args.rush_resume:
        ...
        def drop_unfit_weights(state_dict):
            ...
            # Conditioning-encoder mismatch (e.g. resuming a T5 checkpoint with Ct5=2048 under
            # MorphEm conditioning with Ct5=384): drop the whole conditioning stack and the
            # uncond embedding, keeping the backbone. Guarded so checkpoints that never had
            # these keys don't KeyError.
            ckpt_kv = state_dict.get('text_proj_for_sos.ca.mat_kv.weight')
            if ckpt_kv is not None and ckpt_kv.shape != gpt_wo_ddp.text_proj_for_sos.ca.mat_kv.weight.shape:
                print(f'[resume] conditioning dim changed {tuple(ckpt_kv.shape)} -> '
                      f'{tuple(gpt_wo_ddp.text_proj_for_sos.ca.mat_kv.weight.shape)}; dropping cond weights')
                state_dict.pop('cfg_uncond', None)
    ...
    if args.cond_type == 'morphem':
        # Image conditioning: MorphEm replaces the text encoder entirely. text_tokenizer stays
        # None; text_encoder holds the frozen MorphEm ViT, so the (tokenizer, encoder) plumbing
        # through train_one_ep is unchanged.
        from infinity.models.morphem_encoder import MorphEmEncoder
        print(f'Loading MorphEm from {args.morphem_path}...')
        text_tokenizer = None
        text_encoder = MorphEmEncoder(
            model_path=args.morphem_path,
            img_size=args.morphem_img_size,
            normalize=args.morphem_normalize,
            saturation_noise=bool(args.morphem_saturation_noise),
            device=args.device,
        )
        assert text_encoder.cond_dim == args.Ct5, f'{text_encoder.cond_dim=} != {args.Ct5=}'
    elif args.online_t5:
...
def main_train(args: arg_util.Args):
    ...
    from trainer import InfinityTrainer
    ret: Tuple[
        # tokenizer is None and encoder is a MorphEmEncoder when args.cond_type == 'morphem'
        misc.TensorboardLogger, Optional[T5TokenizerFast], Any, InfinityTrainer,
    ...
    with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=20, verbose=True) as telling_dont_kill:
        ...
        for it, data in me.log_every(start_it, iters_train, ld_or_itrt, args.log_freq, args.log_every_iter, header):
            ...
            with maybe_record_function('before_train'):
                ...
                if args.cond_type == 'morphem':
                    # Image conditioning: encode the image with MorphEm (one CLS token per
                    # channel, bag-of-channels) and use those tokens as the cond sequence.
                    # `captions` is ignored. The conditioning view is derived from the
                    # generation target for now; a dedicated view can come from the dataloader.
                    cond_src = inp if args.morphem_cond_channels <= 0 else inp[:, :args.morphem_cond_channels]
                    cond_img = morphem_encoder_mod.gen_img_to_cond_img(cond_src)
                    text_cond_tuple = text_encoder(cond_img)
                else:
