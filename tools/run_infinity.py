...
def load_morphem_encoder(morphem_path='CaicedoLab/MorphEm', img_size=224, normalize='morphem',
                         saturation_noise=True, device='cuda'):
    """Load the frozen MorphEm image encoder used for image conditioning (replaces load_tokenizer)."""
    from infinity.models.morphem_encoder import MorphEmEncoder
    print(f'[Loading MorphEm image encoder]')
    return MorphEmEncoder(model_path=morphem_path, img_size=img_size, normalize=normalize,
                          saturation_noise=saturation_noise, device=device)


def load_cond_image(image, device='cuda'):
    """Load a conditioning image into a (1, C, H, W) float tensor in the [0, 255] range.

    [0, 255] is what MorphEm's official preprocessing expects -- its SaturationNoiseInjector
    keys off `pixel == 255`. Accepts a path, a numpy array, or a torch tensor; a tensor or
    array already in [-1, 1] or [0, 1] is rescaled to [0, 255].
    Channels are preserved as-is -- MorphEm encodes each one separately (bag of channels).
    """
    if isinstance(image, (torch.Tensor, np.ndarray)):
        t = image.float() if isinstance(image, torch.Tensor) else torch.from_numpy(image).float()
        lo, hi = float(t.min()), float(t.max())
        if lo < -0.01:              # [-1, 1] convention, as produced by the dataloaders
            t = (t + 1) * 127.5
        elif hi <= 1.01:            # [0, 1] convention
            t = t * 255.
    else:
        arr = np.array(PImage.open(image))
        src_dtype = arr.dtype
        if arr.ndim == 2:           # single-channel microscopy image
            arr = arr[:, :, None]
        t = torch.from_numpy(arr.astype(np.float32)).permute(2, 0, 1)   # HWC -> CHW
        if np.issubdtype(src_dtype, np.integer) and np.iinfo(src_dtype).max != 255:
            t = t * (255.0 / float(np.iinfo(src_dtype).max))            # e.g. uint16 -> [0, 255]
    if t.dim() == 3:
        t = t.unsqueeze(0)
    assert t.dim() == 4, f'expected (B, C, H, W), got {tuple(t.shape)}'
    return t.clamp(0, 255).to(device)


def encode_cond_image(morphem_encoder, image):
    """Encode a conditioning image into Infinity's conditioning tuple (the image analogue of encode_prompt)."""
    cond_img = load_cond_image(image)
    cond_tuple = morphem_encoder(cond_img)
    print(f'cond image: {tuple(cond_img.shape)} -> {cond_tuple[0].shape[0]} CLS token(s) of dim {cond_tuple[0].shape[1]}')
    return cond_tuple


def gen_one_img_from_cond_image(
    infinity_test,
    vae,
    morphem_encoder,
    cond_image,
    ...
    """Image-conditioned generation: MorphEm CLS tokens of `cond_image` drive the model.

    Mirrors gen_one_img, with the text prompt replaced by a conditioning image. When
    negative_cond_image is None, CFG uses the model's learned unconditional embedding.
    """
    sstt = time.time()
    ...
    cond_tuple = encode_cond_image(morphem_encoder, cond_image)
    negative_label_B_or_BLT = encode_cond_image(morphem_encoder, negative_cond_image) if negative_cond_image is not None else None
